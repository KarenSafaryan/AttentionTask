...
c = 1;
figure(5)
if strcmp(daq.BoardNames{1},'PCI-6229') % if on recording rig
    xx = 1; % set windows at a certain place
    yy = 500;
    computerid = 0;
elseif max(size(daq.BoardNames))>2 % if on the 2p rig
    xx = -1920;
    yy = 500;
    computerid = 0;
else % if a training rig
    computerid = 1;
    xx = -1599;
    yy = 680;
end
...
if append == 1
    ...
    load(outputFile,'tally','temptnum','lickSpikes','dprimebin','dprime','tallybin','lickSpikearray','disttally',...
        'hit','miss','cr','fa','contally','conhist','laststep','c')
...
while trialnumber<=numtrials
    ...
    if get(handles.psycht,'value')==1 && experimentinfo(5) ==1
        if trialnumber ==1
            contrasts = [25 10 8 6 4 2 0];%[100 50 25 15 12 8 6 4 2];  %SET CONTRASTS HERE  ///////////////////////////////////////////////////////////////////
            laststep =1; %keeps track of the last time contrast changed
            
        end
        if get(handles.ipsycht,'value')==0
            
            randc = mod(round(rand*100),max(size(contrasts)))+1;
            if max(contally)>min(contally)+2
                [~, randc] = min(contally);
            end
            amplitude = contrasts(randc)/200;
            contally(randc) = contally(randc)+1;
            
        elseif get(handles.ipsycht,'value')==1
            if trialnumber==(laststep+30)
                if dprimebin(size(dprimebin,1))>1.75
                    if c<numel(contrasts)
                        c=c+1;
                    end
                    laststep = trialnumber;
                else
                    if c>1
                        c = c-1;
                    end
                ...
                trials = cat(1,['contrast = ' num2str(contrasts(c))],trials);
            end
            amplitude = contrasts(c)/200;
        end
        conhist= [conhist; amplitude*200];
    end
