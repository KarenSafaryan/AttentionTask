...
function main(gratinginfo, waterDispenserinfo, experimentinfo, auditoryinfo,outputfileinfo,outputFile,handles)
    ...
    binsize = 30;
    daq1 = daqhwinfo('nidaq');
    if ~strcmp(daq1.BoardNames{1},'myDAQ1')
        device = 'Dev1';
    else device = 'myDAQ1'; 
    end
    ...
    %Screen Parameters
    if strcmp(daq1.BoardNames{1},'PCI-6229')
        screenid = 1;
    else
...
%***********************ANALOG OUTPUT***************************   
    ...
    Aoutput = daq.createSession('ni');
    Aoutput.addAnalogOutputChannel(device,0:1,'Voltage');
    Aoutput.Rate = 8000; %sample rate
    actualRate = 8000;
    ...
    dio.addDigitalChannel(device,'Port0/Line0:1','OutputOnly');
    dio.addDigitalChannel(device,'Port0/Line3','OutputOnly');
    % dio 03 --> vac
%**************************SOUND CARD*****************************    
...
%**************************ANALOG INPUT***************************   
    ...
    AI.addAnalogInputChannel(device,'ai0','Voltage');
    AI.Rate = 250;
    AI.DurationInSeconds = duration;
    ActualRate = 250;
    durationFrames = duration*ActualRate; %duration in frames
%     set(AI,'SamplesPerTrigger',durationFrames)
%     set(AI,'TriggerType','Manual')
    lh = AI.addlistener('DataAvailable', @call);
...
%************************INITIALIZE SOME VARS*******************
    ...
    trials = {};
    figure(5)
    if strcmp(daq.BoardNames{1},'PCI-6229') || strcmp(daq.BoardNames{1},'myDAQ1')
        xx = 1;
        yy = 500;
    else
        xx = -1599;
        ...
        if experimentinfo(4) == 0         %REGULAR MODE
            ...
            if orientationKey == 1 %non-preferred condition
                ...
                if experimentinfo(5)==1 && experimentinfo(6)==1
                    ...
                    if experimentinfo(7) == 0
                        ...
                        AOutput.queueOutputData([data4 data3]);
                    ...
                    %visual distractor, auditory stim determines water
                        ...
                        if randdist ==0
                            dpref = dpref + 1;
                            dnpref = 0;
                            AOutput.queueOutputData([data4 data2]);
                ...
                elseif experimentinfo(6)==1   %just auditory task
                    putdata(descSound,[nonPrefData nonPrefData]);
                    AnglefromMouse = angle+90;
                    AOutput.queueOutputData([data4 data4]);
                end
            ...
            elseif orientationKey == 0 %preferred condition
                ...
                if experimentinfo(5)==1 && experimentinfo(6)==1
                    ...
                    if experimentinfo(7) == 0
                        AnglefromMouse = angle;
                        AOutput.queueOutputData([data1 data2]);
                    ...
                    %Visual distractor, auditory stim determines water distribution
                        ...
                        if randdist ==0  %preferred
                            ...
                            AOutput.queueOutputData([data1 data3]);
                            AnglefromMouse = angle+90;
                            dnpref = dnpref +1;
                            dpref = 0;
                            vdnpref = vdnpref+1;
                ...
                elseif  experimentinfo(6) == 1  %auditory task
                    putdata(descSound,[prefData prefData]);
                    AnglefromMouse = angle;
                    AOutput.queueOutputData([data1 data4]);
                end
...
%                 Screen('FillRect',win,[250 250 250],[0 0 50 50]);
        ...
        if experimentinfo(6)==1
            dio.outputSingleScan([0 1 1]);
        else
            dio.outputSingleScan([0 0 1]);
        end
        Screen('FillRect',win,[250 250 250], [0 0 50 50]);
        Screen('Flip', win);
